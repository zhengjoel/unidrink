			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				...
				<u-form :model="form" ref="uForm" label-position="top">
					<u-form-item label="购买数量" labelWidth="150rpx">
						<u-number-box v-model="form.stock" :min="1" :max="product.stock" @change="valChange"></u-number-box>
					</u-form-item>
	...
	export default {
		...
		methods: {
			...
			async submit() {
				...
				let res = {}
				if (this.type == 3) {
					res = await this.$u.api.greenCreate({
						id: this.id,
					...
					if (res.code == 1) {
						let that = this;
						setTimeout(function() {
							that.$u.route('/pages/store_creen/order')
						}, 1500)
					}
				} else if (this.type == 2) {
					res = await this.$u.api.scoreCreate({
						id: this.id,
						addressId: this.form.address.id,
						num: this.form.num,
						payword: this.form.payword
					})
					this.$refs.uToast.show({
						title: res.msg,
						type: res.code == 1 ? 'success' : 'warning',
					...
					if (res.code == 1) {
						let that = this;
						setTimeout(function() {
							that.$u.route('/pages/order/order?current=1')
						}, 1500)
					}
				} else if (this.type == 1) {
					res = await this.$u.api.unionCreate({
						id: this.id,
						addressId: this.form.address.id,
						num: this.form.num,
						payword: this.form.payword
