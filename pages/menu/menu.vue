<template>
	<view class="container" v-if="!loading">
		<view class="main">
			...
			<!-- #ifdef H5 -->
			<view class="content"
				:style="{height: 'calc(100vh - 200rpx - 100rpx + '+(store.notice ? '0rpx':'60rpx')+')'}">
				<!-- #endif -->
				<!-- #ifndef H5 -->
...
<script>
	...
	export default {
		...
		computed: {
			...
			disabledPay() { //是否达到起送价
				return this.orderType == 'takeout' && (this.getCartGoodsPrice < parseFloat(this.store.min_price)) ? true :
					false
			},
			spread() { //差多少元起送
